...
| Case | Setup | Required detection response |
|---|---|---|
| **Parent overlay defeats the settings read** (CR-38/CR-42) | Project config `apply: false`; launch the parent with `--config` setting `apply: true`; run `/orchestrated` on independent parallel scope | Subprocess `omp config get` reports `false` (a false pass on its own). The **same-session canary** detects the merge-summary does NOT begin with "Isolation:", fails the preflight, and blocks fan-out. The canary is a **behavioral guard** (effective tool surface is `[read, hub]` per CR-44 — see §08 §E-9.2); it MUST NOT directly write files, and its prompt instructs "make no changes". Under `apply=false` isolation, any isolated-worktree changes are never applied to the parent, so the parent-tree invariant holds by construction. **Fanning out because the diagnostic passed is a FAIL. A canary that directly mutates the parent is also a FAIL.** Repeat with an in-session `/settings` override, where no file changes at all. |
