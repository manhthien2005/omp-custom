# Prompt for Codex Peer Review — Adversarial Audit of Topic 01

You are a fresh Codex review session acting as an independent technical peer on `omp-template`,
not as the author of the Topic 01 changes. The user explicitly authorized Codex as the temporary
reviewer because Claude Opus has no available account/quota. This substitution changes only the
reviewer identity; it does not weaken the frozen contract or acceptance gate.

Review only from the user-approved contract, repository files, and pinned OMP source evidence.
Ignore any prior author conclusion and attempt to falsify the work independently.
...
1. Read `opus5-review-packet-codex-topic01-optimization-metrics.md` first. Its filename records
   the originally intended reviewer; its substantive contract remains the frozen audit input.
