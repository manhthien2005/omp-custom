...
One such revision has occurred: the owner-authorized `README.md` redesign (Oh My Pi link corrected
to `https://github.com/can1357/oh-my-pi`, plus a visual restructure), which forced its two governed
hash pins to be regenerated through the sanctioned capture scripts.

```text
pwsh -File scripts/repair-evidence-byte-integrity.ps1
  FAIL: Exact=966 Recoverable=0 Unrecoverable=5 Missing=0 Invalid=4
        Ambiguous=0 Written=0                                                       exit 1
```

The four `invalid` paths are exactly the authorized change set, and `966 + 4 = 970` — the previous
`Exact` count — so no other snapshot row drifted:
